...
- `OmniArticleMarkdown` in `src/omni_article_markdown/omni_article_md.py` is the orchestrator. Its flow is fixed: `ReaderFactory` fetches HTML, `ExtractorFactory` turns that HTML into an `Article`, and `HtmlMarkdownParser` converts the cleaned article body into the final Markdown output.
...
- Treat `.agents/skills/reader-developer/SKILL.md` and `.agents/skills/extractor-developer/SKILL.md` as task-specific playbooks when working on those areas. They document the expected investigation flow for acquiring HTML and for refining site-specific extraction. (Claude Code also resolves these via `.claude/skills`.)
- Keep `can_handle()` implementations narrow because factory selection stops at the first match. Readers typically match on URL prefixes; extractors often match on OG/canonical metadata helpers from `src/omni_article_markdown/utils.py` such as `get_og_*()`, `get_canonical_url()`, and `is_matched_canonical()`.
...
- **读取角色技能**：在`.agents/skills/`目录下找到角色对应的技能
